...
void showFlightCancelReservationMenu()
{
	...
	for (int i = 1; i <= flights.length(); i++)
	{
		...
		if (inFlight || inQueue)
		{
			if (inQueue) cout << "(QUEUED) "; // ’ν είναι στην ουρά τότε εκτυπώνουμε αυτό το πρόθεμα.
		...
		while (!validFlight)
		{
			...
			if (passengerIndex != 0 || inQueue)
			{
				...
				if (passengerIndex)
				{
					...
					{
						// ’ρα ο αρχαιότερος στην ουρά μπορεί να έχει θέση, οπότε μπαίνει στη νέα λίστα επιβατών.
						LinkedQueue<Passenger> queue = flight.getQueue();
						Passenger first;
						queue - first;
